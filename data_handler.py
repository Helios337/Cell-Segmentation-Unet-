...
class BBBCDataLoader:
    ...
    def download_dataset(self, save_dir="./data"):
        """
        Downloads a BBBC dataset zip file[cite: 26].
        """
        ...
        if os.path.exists(zip_path):
            ...
            try:
                response = requests.get(zip_url, stream=True, timeout=30) [cite: 28]
                if response.status_code == 200:
                    with open(zip_path, "wb") as f:
                        for chunk in response.iter_content(chunk_size=8192):
                            f.write(chunk) [cite: 29]
    ...
    def load_synthetic_data(self, n_samples=100, img_size=(256, 256)):
        """Generates synthetic cell-like data for demonstration[cite: 31]."""
