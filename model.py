...
class CellSegmentationTool:
    """
    A comprehensive tool for cell segmentation using a U-Net model[cite: 2].
    Encapsulates model building, training, prediction, and evaluation[cite: 2].
    """
    ...
    def build_unet(self):
        ...
        c1 = layers.Dropout(0.1)(c1) [cite: 4]
        ...
        c3 = layers.Dropout(0.2)(c3) [cite: 5]
        ...
        # Bottleneck
        c5 = layers.Conv2D(1024, (3, 3), activation='relu', kernel_initializer='he_normal', padding='same')(p4) [cite: 6]
        ...
        c6 = layers.Conv2D(512, (3, 3), activation='relu', kernel_initializer='he_normal', padding='same')(c6) [cite: 7]
        ...
        u8 = layers.concatenate([u8, c2]) [cite: 8]
        ...
        c9 = layers.Conv2D(64, (3, 3), activation='relu', kernel_initializer='he_normal', padding='same')(c9) [cite: 9]
    ...
    def dice_coef(self, y_true, y_pred, smooth=1e-6):
        ...
        intersection = tf.keras.backend.sum(y_true_f * y_pred_f) [cite: 10]
        return (2. * intersection + smooth) / (tf.keras.backend.sum(y_true_f) + tf.keras.backend.sum(y_pred_f) + smooth) [cite: 11]
    ...
    def compile_model(self):
        """Compiles the U-Net model with optimizer, loss, and metrics[cite: 12]."""
    ...
    def train_model(self, X_train, y_train, X_val, y_val, epochs=50, batch_size=8):
        ...
        callbacks = [
            ...
            keras.callbacks.ModelCheckpoint('best_model.h5', save_best_only=True, monitor='val_loss') [cite: 14]
        ]
    ...
    def predict_segmentation(self, image):
        """
        Predicts the raw segmentation mask for a single image[cite: 15].
        This method now correctly returns only the raw probability map[cite: 16].
        """
        if self.model is None:
            raise ValueError("Model has not been trained or loaded.") [cite: 17]
    ...
    def post_process_and_count(self, mask, threshold=0.5, min_size=50):
        """
        This new method contains the post-processing and counting logic[cite: 18].
        It takes a raw prediction mask as input[cite: 19].
        """
        ...
        coords = np.argwhere(morphology.local_maxima(distance)) [cite: 20]
    ...
    def visualize_results(self, image, true_mask, raw_prediction, labeled_mask, cell_count):
        ...
        Raw Prediction, and Final Labeled Segmentation with the count[cite: 21, 22].
        """
        ...
        axes[2].axis('off')
        colored_labels = plt.cm.nipy_spectral(labeled_mask / (labeled_mask.max() or 1)) [cite: 23]
    ...
    def evaluate_model(self, X_test, y_test):
        """Evaluates the model's performance on the test set[cite: 23, 24]."""
